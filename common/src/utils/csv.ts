...
export function save(filePath, lines, cols, sortBy,  setActionMessage) {
  const sortedLines = sortByCol(lines, sortBy);
...
export function saveAs(currentFile, setCurrentFile, fileChange, lines, cols, sortBy, setActionMessage) {
  const { getCurrentWindow, dialog } = window.require('@electron/remote');

  return new Promise((resolve, reject) => {
    const filePath = dialog.showSaveDialogSync(getCurrentWindow(), {
    ...
    if (filePath) { // if user cancelled, filePath is undefined
      save(filePath, lines, cols, sortBy, setActionMessage)
        .then(sortedLines => {
